  ...
  const formato = req.query.formato || "json";
    ...
    if (formato ==='sql') {
      const values = lista.map( p => {
        return `('${p.nomecompleto}', '${p.cpf}', '${p.nomepai}','${p.nomemae}', '${p.data_nascimento}','${p.idade}','${p.enderecofinal}','${p.bairro}','${p.cidade}','${p.estado}','${p.cep}','${p.naturalidade}','${p.nacionalidade}')`;
      }).join(',\n');
